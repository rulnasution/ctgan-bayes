...
class BayesCTGANSWA(BaseSynthesizer):
    ...
    def __init__(self, embedding_dim=128, generator_dim=(256, 256), discriminator_dim=(256, 256),
                 ...
                 discriminator_decay=1e-6, cyclic_lr = False,
                 batch_size=500, 
                 n_test_eval = 5000, country = 'Canada',
                 full_eval_every = 25, bma_steps = [1, 2], cov_scales = [0, 0.5],
        ...
        self.bma_steps = bma_steps 
        self.cov_scales = cov_scales
    ...
    def evaluate_test_compre(self, n_data, country, bma_steps = [1], cov_scales = [0.5]):
        ...
        temp_swag_model = deepcopy(self.swag_model)
        temp_generator = deepcopy(self.generator)
        results = []
        # print(self.eval_swa)
        ## CTGAN
        ...
        if self.eval_swa:
            # print(real.head())
            # print(real.info())
            if 0 in cov_scales: ## SWA
                real = deepcopy(self.train_data) if n_data == len(self.train_data) else deepcopy(real2)
                cov_scales = cov_scales[1:] if len(cov_scales) > 1 else None
                fake = self.sample(n_data, 1, 0.0, False, False, False)
                eval_res = self.evaluate_test(real, fake, country)
                results.append(['SWA', 1, 0.0, False, False, False] + eval_res)
            
            if bma_steps is not None and cov_scales is not None:
                for bma_step in bma_steps: ## SWAG
                    # print(real.head())
                    # print(real.info())
                        ...
                        for cov1 in [True, False]:
                            for block in [True, False]:
                                for bnu in [True, False]:
                                    real = deepcopy(self.train_data) if n_data == len(self.train_data) else deepcopy(real2)
                                    fake = self.sample(n_data, bma_step, scale, cov1, block, bnu)
                                    eval_res = self.evaluate_test(real, fake, country)
                                    results.append(['SWAG', bma_step,scale,cov1,block,bnu] + eval_res)

        # print(real.head())
        # print(real.info())
        real = deepcopy(self.train_data) if n_data == len(self.train_data) else deepcopy(real2)
        fake = self.sample(n_data, 1, 0.0, False, False, False, False) ## use_swag = False
        eval_res = self.evaluate_test(real, fake, country)
        results.append(['CTGAN', 1, 0.0, False, False, False] + eval_res)
            
        results1 = pd.DataFrame(results)
        results1.columns = ['method','bma_step','cov_scale','covariance','cov_block','bn_update',
                            'roc_uni','roc_bi', 'cio','utility','risk']
        
        self.swag_model = deepcopy(temp_swag_model)
        self.generator = deepcopy(temp_generator)
        return results1 ## bivariate roc, univariate roc, cio, tcap
    ...
    def fit(self, train_data, discrete_columns=(), epochs=None):
        ...
        optimizerG = self.generator_opt(
            self.generator.parameters(), lr=self._generator_lr,
            betas=(0.5, 0.9), weight_decay=self._generator_decay if self.prior_var is None else 0.0)
        ...
        self.swag_model = SWAG(Generator(
                               ...
                               no_cov_mat= not self.swa_cov_mat,
        ...
        # gnoise_criterion = NoiseLoss(params=self.generator.parameters(), 
        ...
        for i in epoch_iterator:
            ...
            for id_ in range(steps_per_epoch):
                for n in range(self._discriminator_steps):
                    ...
                    y_fake = self.discriminator(fake_cat.detach())
                    ...
                    if self.loss_discriminator=='wasserstein':
                        ...
                        loss_d = -(torch.mean(y_real) - torch.mean(y_fake)) + pen # last used was without negative, changed in 23/9/2024
                ...
                if self.loss_discriminator=='wasserstein':
                    loss_g += -torch.mean(y_fake) # last used was without negative, changed in 23/9/2024
                elif self.loss_discriminator=='f-totalvar':
                    """ Total Variation """
                    ...
                    if self.loss_discriminator=='ls':
                        loss_g += (loss_func(y_fake, real_label)/2)
                    else:
                        loss_g += loss_func(y_fake, real_label)
                
                ## the cross entropy loss is added in the end.
                loss_g += cross_entropy

                if self.swag and self.prior_var is not None:
                    # print(get_prior(generator.parameters(), self._batch_size))

                    loss_g += gprior_criterion(self.generator.parameters())
            ...
            if (i + 1) >= self.swa_start:
                self.eval_swa = True
                # if self.swag: 
                self.swag_model.collect_model(self.generator)
            ...
            if self.save_results:
                if ((i+1)%self.full_eval_every ==0 or i in [0, epochs-1]):
                    eval_current = self.evaluate_test_compre(len(train_data), self.country, self.bma_steps, self.cov_scales) ## [utility, risk, improvement score]
                else:
                    eval_current = self.evaluate_test_compre(self.n_test_eval, self.country, self.bma_steps, self.cov_scales) if self.n_test_eval is not None else None
                # print(eval_current)
                if eval_current is not None:
                    ...
                    self.evaluation_results.to_csv(f'{self.save_folder}/SWAG_eval_results_{self.country}_{self.loss_discriminator}_{str(self.swa_max_models_save)}_{str(self._epochs)}.csv', index=False)
            # if self.save_results: 
    ...
    @random_state
    def sample(self, n, bma_step = 1, scale = 1.0, cov = True, block = False, update_bn = False, use_swag = True,
