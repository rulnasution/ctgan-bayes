...
if __name__ == '__main__':
    ...
    parser.add_argument("--id_data", type=int, default=1)
    ...
    phi_val = args.phi ## the weight for utility
    # for i in range(9):
    for loss in ['wasserstein','vanilla']: # ,'wasserstein','ls'
        for seed in range(1,2):
            try:
                print(country_names[i],loss, seed, args.epochs, args.swa_rank, args.swa_start)
                delu.random.seed(seed)
                ctgan = BayesCTGANSWA(epochs=2, loss_discriminator=loss,
                            verbose=False, pac=10, 
                            # full_eval_every = 10,
                            # bma_steps = [1, 2], cov_scales = [0, 0.25, 0.5, 1],
                            n_test_eval = None, country = country_names[i],
                            swa_start = args.swa_start, swa_lr = 2e-3, cyclic_lr = True, swa_collect = 1, 
                            swa_cov_mat = True, swag = True,
                            swa_max_models_save = args.swa_rank, save_results = False,
                            # save_folder = init_dir + '/simulation/100-every10/' + str(seed)
                            )
                ctgan.fit(dfs[i], discrete_columns[i])

                # ctgan._batch_size
                ctgan.swag_model.load_state_dict(torch.load(f'{init_dir}/{args.model_folder}/model_gan_swag_{country_names[i]}_{loss}_{args.swa_rank}_{seed}_genswag.pt',map_location="cpu"))
                ctgan.generator.load_state_dict(torch.load(f'{init_dir}/{args.model_folder}/model_gan_swag_{country_names[i]}_{loss}_{args.swa_rank}_{seed}_gen.pt',map_location="cpu"))
                ctgan.discriminator.load_state_dict(torch.load(f'{init_dir}/{args.model_folder}/model_gan_swag_{country_names[i]}_{loss}_{args.swa_rank}_{seed}_dis.pt',map_location="cpu"))
                ...
                if args.swa_rank == 30:
                    ...
                    for s in range(20):
                        delu.random.seed(s)
                        dt1 = ctgan.sample(len(dfs[i]), 1, 0.0, False, False, False, use_swag=False) 
                        ...
                        final_res = pd.concat([final_res,
                                    ...
                                    'GAN',loss,'CTGAN',seed,1,0.0,False, False, False,
                                    s,
                        ...
                        print([[country_names[i],
                                    ...
                                    s,
                                    roc_val[0], roc_val[1], cio_val,
                                    (roc_val[0]+roc_val[1]+cio_val)/3,tcap_val]])
                        dt1.to_csv(f'{init_dir}/synth_data/data_gan_swag_CTGAN_{loss}_{country_names[i]}_{args.swa_rank}_{s}.csv', index=False)
                    final_res.to_csv(f'simulation/200-epochs-50-from-model-s1-p1-20/results_gan_swag_{country_names[i]}_{args.epochs}_{args.swa_rank}.csv', index=False)
                    
                    ## use_swag default in ctgan-swa.sample = True 
                    ## SWA
                    for s in range(20):
                        delu.random.seed(s)
                        dt1 = ctgan.sample(len(dfs[i]), 1, 0.0, False, False, False)
                        roc_val = cal_mean_roc(country_names[i],dfs[i],dt1)
                        cio_val = cal_mean_cio(country_names[i],dfs[i],dt1)
                        tcap_val = cal_mean_tcap(country_names[i],dfs[i],dt1)
                        final_res = pd.concat([final_res,
                                                pd.DataFrame([[country_names[i],
                                    'GAN',loss,'SWA',seed,1,0.0,False, False, False,
                                    s,
                                    roc_val[0], roc_val[1], cio_val,
                                    (roc_val[0]+roc_val[1]+cio_val)/3,tcap_val]])],axis=0)
                        print([[country_names[i],
                                    ...
                                    roc_val[0], roc_val[1], cio_val,
                                    (roc_val[0]+roc_val[1]+cio_val)/3,tcap_val]])
                        dt1.to_csv(f'{init_dir}/synth_data/data_gan_swag_SWA_{loss}_{country_names[i]}_{args.swa_rank}_{s}.csv', index=False)
                    final_res.to_csv(f'simulation/200-epochs-50-from-model-s1-p1-20/results_gan_swag_{country_names[i]}_{args.epochs}_{args.swa_rank}.csv', index=False)
                    

                    ## SWAG Diagonal = no covariance
                    best_val = 0
                    for bma_step in [1, 2]:
                                ...
                                for block in [False]:
                                    for bnu in [True, False]:
                                        ...
                                        for s in range(20):
                                            ...
                                            final_res = pd.concat([final_res,
                                                                    pd.DataFrame([[country_names[i],
                                                    'GAN',loss,'SWAGd',seed,bma_step,scale,cov1,block,bnu,
                                                    s,
                                                    roc_val[0], roc_val[1], cio_val,
                                                    (roc_val[0]+roc_val[1]+cio_val)/3,tcap_val]])],axis=0)
                                            dt1.to_csv(f'{init_dir}/synth_data/temp/data_gan_swag_{country_names[i]}_{args.swa_rank}_{s}.csv', index=False)
                                            u1 = (roc_val[0]+roc_val[1]+cio_val)/3
                                            r1 = max(tcap_val, 0)
                                            util.append(u1)
                                            risk.append(r1)

                                        final_res.to_csv(f'simulation/200-epochs-50-from-model-s1-p1-20/results_gan_swag_{country_names[i]}_{args.epochs}_{args.swa_rank}.csv', index=False)
                                        ss = phi_val*np.mean(util) + (1-phi_val)*(1-np.mean(risk))
                                        if ss > best_val:
                                            best_val = ss
                                            print([[country_names[i],
                                                    'GAN',loss,'SWAGd',seed,bma_step,scale,cov1,block,bnu,
                                                    s,np.mean(util),np.mean(risk),util,risk]])
                                            for s in range(20):
                                                dt1 = pd.read_csv(f'{init_dir}/synth_data/temp/data_gan_swag_{country_names[i]}_{args.swa_rank}_{s}.csv')
                                                dt1.to_csv(f'{init_dir}/synth_data/data_gan_swag_SWAGd_{loss}_{country_names[i]}_{args.swa_rank}_{s}_best_param.csv', index=False)
                
                ## SWAG with approximate covariance
                best_val = 0
                for bma_step in [1, 2]:
                    for scale in [0.25, 0.5, 1.0]:
                        for cov1 in [True]:
                            for block in [True, False]:
                                for bnu in [True, False]:
                                    ...
                                    for s in range(20):
                                        ...
                                        final_res = pd.concat([final_res,
                                                                pd.DataFrame([[country_names[i],
                                                'GAN',loss,'SWAG',seed,bma_step,scale,cov1,block,bnu,
                                                s,
                                                roc_val[0], roc_val[1], cio_val,
                                                (roc_val[0]+roc_val[1]+cio_val)/3,tcap_val]])],axis=0)
                                        dt1.to_csv(f'{init_dir}/synth_data/temp/data_gan_swag_{country_names[i]}_{args.swa_rank}_{s}.csv', index=False)
                                    ...
                                    if ss > best_val:
                                        ...
                                        for s in range(20):
                                            dt1 = pd.read_csv(f'{init_dir}/synth_data/temp/data_gan_swag_{country_names[i]}_{args.swa_rank}_{s}.csv')
                                            dt1.to_csv(f'{init_dir}/synth_data/data_gan_swag_SWAG_{loss}_{country_names[i]}_{args.swa_rank}_{s}_best_param.csv', index=False)
